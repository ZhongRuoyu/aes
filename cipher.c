...
word *Cipher(unsigned Nb, unsigned Nr, const word in[], word **key) {
    ...
    for (unsigned j = 0; j < Nb; ++j) {
        state[j] =
            s_box[0][prev[(j + 0) % Nb].bytes[3]] ^
            s_box[1][prev[(j + 1) % Nb].bytes[2]] ^
            s_box[2][prev[(j + 2) % Nb].bytes[1]] ^
            s_box[3][prev[(j + 3) % Nb].bytes[0]] ^
            key[Nr][j];
    }
...
word *InvCipher(unsigned Nb, unsigned Nr, const word in[], word **key) {
    ...
    for (unsigned j = 0; j < Nb; ++j) {
        state[j] =
            inverse_s_box[0][prev[(j + 4) % Nb].bytes[3]] ^
            inverse_s_box[1][prev[(j + 3) % Nb].bytes[2]] ^
            inverse_s_box[2][prev[(j + 2) % Nb].bytes[1]] ^
            inverse_s_box[3][prev[(j + 1) % Nb].bytes[0]] ^
            key[0][j];
    }
